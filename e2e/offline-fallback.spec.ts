...
test.describe('在线房间断网回退 E2E', () => {
  test('断网打开在线房间 → 显示本地缓存而非转为过期，联网后列表无重复', async ({ page }) => {
    ...
    // postgrest 默认对网络错误重试（1s/2s/4s），等待最终回退到本地缓存
    await expect(page.getByText('网络异常，正在显示本地缓存（只读）')).toBeVisible({ timeout: 25000 })
    expect(await roomMode(page, roomId)).toBe('online')
    expect(await expiredRooms(page)).toEqual([])

    // 联网刷新首页：只有一条在线条目，不再出现过期只读重复项
